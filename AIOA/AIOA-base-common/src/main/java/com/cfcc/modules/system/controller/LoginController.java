...
//        }
    ...
    public Result<JSONObject> login(@RequestBody SysLoginModel sysLoginModel, HttpServletRequest request) throws Exception {
        ...
        //用户登录信息
        sysUser.setOrgSchema(orgSchema);
        userInfo(sysUser, result);
        sysBaseAPI.addLog("用户名: " + username + ",登录成功！", CommonConstant.LOG_TYPE_1, null);
    ...
    public Result<Object> logout(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (sysUser != null) {
            ...
            //清除存储的用户信息
            redisUtil.del(CommonConstant.LOGIN_INFO + token);
    ...
    public Result<JSONObject> selectDepart(@RequestBody SysUser user, HttpServletRequest request) {
        Result<JSONObject> result = new Result<JSONObject>();
        ...
        //获取登录信息
        String token = request.getHeader(DefContants.X_ACCESS_TOKEN);
        String redisKey = CommonConstant.LOGIN_INFO + token;
        LoginInfo loginInfo = (LoginInfo) redisUtil.get(redisKey);
        SysDepart sysDepart = sysDepartService.queryUserDepartByOrgCode(orgCode);
        //得到当前部门-再存入redis
        loginInfo.setDepart(sysDepart);
    ...
    private Result<JSONObject> userInfo(SysUser sysUser, Result<JSONObject> result) {
        ...
        loginInfo.setDeparts(departs);
        obj.put("token", token);
        obj.put("userInfo", sysUser);
        BeanUtils.copyProperties(sysUser, loginInfo);
        //角色
