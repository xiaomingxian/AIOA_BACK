...
public class SysDictController {
     ...
     *
     */
    @GetMapping("/getDictByKeyObj")
    public Result getDictByKeyObj(String dictKey, HttpServletRequest request) {
        try {
     ...
     * 这个查询的是描述
     */
    @GetMapping("/getDictBySqlValue")
    public Result getDictBySqlValue(String dictValue, HttpServletRequest request) {
        try {
    ...
    public Result getDictByKey(String dictKey, HttpServletRequest request) {
        ...
        try {
            ...
            //一般2-3条数据
            List<Map<String, Object>> dictByKey = sysDictService.getDictByKey(dictKey);

            List<SysDictItem> result = new ArrayList<>();

            dictByKey.stream().forEach(map -> {

                String departIds =(String) map.get("departIds") ;
                String dictVal = map.get("dictVal") + "";
                String dictItem = map.get("dictItem") + "";
                String description = map.get("description") + "";
                //如果不需要区分部门--或者已经包含部门
                if (departIds == null || (departIds != null && departIds.contains(deptId))) {
                    SysDictItem sysDictItem = new SysDictItem();
                    sysDictItem.setItemValue(dictVal);
                    sysDictItem.setItemText(dictItem);
                    sysDictItem.setDescription(description);
                    result.add(sysDictItem);
                }
            });
    ...
    @GetMapping("getDictItemText")
    public Result getDictItemText(String dictKey, String itemValue) {

        String itemText = sysDictItemService.getDictItemText(dictKey, itemValue);
        return null;
    }
    ...
    public Result<String> getDictItems(@PathVariable("dictCode") String dictCode, @PathVariable("key") String key) {
        ...
        String text = null;
        try {
            text = sysDictService.queryDictTextByKey(dictCode, key, MycatSchema.getSchema());
            result.setSuccess(true);
