...
public class ReCallTaskServiceImpl implements ReCallTaskService {
    ...
    public void backProcess(String taskId, String activityId, Map<String, Object> variables,
                            SysUser currentUser, String type, String descActName) throws Exception {
        if (StringUtils.isEmpty(activityId)) {
            throw new AIOAException("回退目标节点ID为空！");
        }
        ...
        if ("撤回".equals(type)) {
            ...
            list.stream().forEach(task -> {
                //找到匹配的办理人为他设置assignee
                String id = task.getId();
                if (activityId.equals(task.getTaskDefinitionKey())) {
                    taskService.setAssignee(id, currentUser.getId());
                }
            });
        }
     ...
     * @return 节点流向集合
     */
    private List<PvmTransition
            > clearTransition(ActivityImpl activityImpl) {
        // 存储当前节点所有流向临时变量
